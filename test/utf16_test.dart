// Copyright (c) 2012, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.

library utf.utf16_test;

import 'package:flutter_test/flutter_test.dart';
import 'package:unicode/unicode.dart';

const String testKoreanCharSubset = '''
가각갂갃간갅갆갇갈갉갊갋갌갍갎갏감갑값갓갔강갖갗갘같갚갛
개객갞갟갠갡갢갣갤갥갦갧갨갩갪갫갬갭갮갯갰갱갲갳갴갵갶갷
갸갹갺갻갼갽갾갿걀걁걂걃걄걅걆걇걈걉걊걋걌걍걎걏걐걑걒걓''';

const String testHanWater = '水';

const testKoreanCharSubsetUtf16beBom = <int>[
  0xfe, 0xff, 0xac, 0x00, 0xac, 0x01, 0xac, 0x02, // 8
  
];

const testKoreanCharSubsetUtf16le = <int>[
  0x00, 0xac, 0x01, 0xac, 0x02, 0xac, 0x03, 0xac, // 8
  
];

void main() {
  test('encode to utf16', testEncodeToUtf16);
  test('utf16 bytes to string', testUtf16BytesToString);
}

void testEncodeToUtf16() {
  Utf16Encoder encoder = utf16.encoder as Utf16Encoder;

  // TODO(dcarlson) should we skip bom if empty?
  expect([], encoder.encodeUtf16Be(''));
  expect(testKoreanCharSubsetUtf16beBom, encoder.convert(testKoreanCharSubset),
      reason: 'encode UTF-16(BE by default) Korean');

  expect(
      testKoreanCharSubsetUtf16le, encoder.encodeUtf16Le(testKoreanCharSubset),
      reason: 'encode UTF-16LE Korean');
}

void testUtf16BytesToString() {
  Utf16Decoder decoder = utf16.decoder as Utf16Decoder;
  expect('', decoder.convert([]));
  expect(testHanWater, decoder.convert([0x6C, 0x34]),
      reason: 'Water variation 1');
  expect(testHanWater, decoder.convert([0xFE, 0xFF, 0x6C, 0x34]),
      reason: 'Water variation 2');
  expect(testHanWater, decoder.convert([0xFF, 0xFE, 0x34, 0x6C]),
      reason: 'Water variation 3');

  expect(testHanWater, decoder.decodeUtf16Be([0x6C, 0x34]),
      reason: 'Water variation 4');
  expect(testHanWater, decoder.decodeUtf16Be([0xFE, 0xFF, 0x6C, 0x34]),
      reason: 'Water variation 5');

  expect(testHanWater, decoder.decodeUtf16Le([0x34, 0x6C]),
      reason: 'Water variation 6');
  expect(testHanWater, decoder.decodeUtf16Le([0xFF, 0xFE, 0x34, 0x6C]),
      reason: 'Water variation 7');

  expect(testKoreanCharSubset, decoder.convert(testKoreanCharSubsetUtf16beBom),
      reason: 'UTF-16BE Korean');
}
