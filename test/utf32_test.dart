// Copyright (c) 2012, the Dart project authors.  Please see the AUTHORS file
// for details. All rights reserved. Use of this source code is governed by a
// BSD-style license that can be found in the LICENSE file.

library utf.utf32_test;

import 'package:flutter_test/flutter_test.dart';
import 'package:unicode/unicode.dart';

const String testKoreanCharSubset = '''
가각갂갃간갅갆갇갈갉갊갋갌갍갎갏감갑값갓갔강갖갗갘같갚갛
개객갞갟갠갡갢갣갤갥갦갧갨갩갪갫갬갭갮갯갰갱갲갳갴갵갶갷
갸갹갺갻갼갽갾갿걀걁걂걃걄걅걆걇걈걉걊걋걌걍걎걏걐걑걒걓''';

const String testHanTwice = '二';

const testKoreanCharSubsetUtf32beBom = <int>[
  0x00, 0x00, 0xfe, 0xff, 0x00, 0x00, 0xac, 0x00, // 8
  
];

const testKoreanCharSubsetUtf32le = <int>[
  0x00, 0xac, 0x00, 0x00, 0x01, 0xac, 0x00, 0x00, // 8
  
];

void main() {
  test('utf32 bytes to string', testUtf32BytesToString);
  test('encode to utf32', testEncodeToUtf32);
}

void testEncodeToUtf32() {
  Utf32Encoder encoder = utf32.encoder as Utf32Encoder;
  expect([], encoder.encodeUtf32Le(''),
      reason: 'no input'); // TODO(dcarlson) skip bom on empty?
  expect(testKoreanCharSubsetUtf32beBom, encoder.convert(testKoreanCharSubset),
      reason: 'encode UTF-32(BE by default) Korean');
  expect(
      testKoreanCharSubsetUtf32le, encoder.encodeUtf32Le(testKoreanCharSubset),
      reason: 'encode UTF-32(LE by default) Korean');
}

void testUtf32BytesToString() {
  Utf32Decoder decoder = utf32.decoder as Utf32Decoder;
  expect('', decoder.convert([]), reason: 'no input');
  expect('\ufffd', decoder.convert([0]), reason: 'single byte');
  expect('\ufffd', decoder.convert([0, 0, 0x4e]), reason: 'short a byte');
  expect('\u4e8c\ufffd', decoder.convert([0, 0, 0x4e, 0x8c, 0]),
      reason: 'extra byte');

  expect(testHanTwice, decoder.convert([0, 0, 0x4e, 0x8c]),
      reason: 'twice variation 1');
  expect(testHanTwice, decoder.convert([0, 0, 0xfe, 0xff, 0, 0, 0x4e, 0x8c]),
      reason: 'twice variation 2');
  expect(testHanTwice, decoder.convert([0xff, 0xfe, 0, 0, 0x8c, 0x4e, 0, 0]),
      reason: 'twice variation 3');

  expect(testHanTwice, decoder.decodeUtf32Be([0, 0, 0x4e, 0x8c]),
      reason: 'twice variation 4');
  expect(
      testHanTwice, decoder.decodeUtf32Be([0, 0, 0xfe, 0xff, 0, 0, 0x4e, 0x8c]),
      reason: 'twice variation 5');

  expect(testHanTwice, decoder.decodeUtf32Le([0x8c, 0x4e, 0, 0]),
      reason: 'twice variation 6');
  expect(
      testHanTwice, decoder.decodeUtf32Le([0xff, 0xfe, 0, 0, 0x8c, 0x4e, 0, 0]),
      reason: 'twice variation 7');

  expect(testKoreanCharSubset, decoder.convert(testKoreanCharSubsetUtf32beBom),
      reason: 'UTF-32BE Korean');
}
